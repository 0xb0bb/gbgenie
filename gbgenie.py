...
def decode(code):
    ...
    if len(code) > 7:
        orig = rr8(int(code[8] + code[10], 16) ^ 0xff, 2) ^ 0x45

    return {'addr': addr, 'value': val, 'original': orig}
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        print 'code: %s' % encode(val, addr, orig)
